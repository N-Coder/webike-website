...
"""value 4 (0 indexed) should be close to value 5 for the transition from 1 to 2 to be smooth. dont want a discontinuity"""
...
"""
linear model. ignore first 5 and lsat 6, cooresponing to modes  1 and 3, when training linear model
"""
linearN20, parm_cov = sp.optimize.curve_fit(model_funcLinear, d["-20"]["Ys"][5:47],
                                            [(d["-20"]["maxwh"] - d["-20"]["Xs"][i] * d["-20"]["Ys"][i] / 1000) /
                                             d["-20"]["maxwh"] for i in range(5, 47)])
linearN10, parm_cov = sp.optimize.curve_fit(model_funcLinear, d["-10"]["Ys"][5:47],
                                            [(d["-10"]["maxwh"] - d["-10"]["Xs"][i] * d["-10"]["Ys"][i] / 1000) /
                                             d["-10"]["maxwh"] for i in range(5, 47)])

"""
three line model
"""
threeLine0, parm_cov = sp.optimize.curve_fit(model_func3Line, d["0"]["Ys"],
                                             [(d["0"]["maxwh"] - d["0"]["Xs"][i] * d["0"]["Ys"][i] / 1000) / d["0"][
                                                 "maxwh"] for i in range(0, len(d["0"]["Xs"]))])
threeLineP23, parm_cov = sp.optimize.curve_fit(model_func3Line, d["23"]["Ys"],
                                               [(d["23"]["maxwh"] - d["23"]["Xs"][i] * d["23"]["Ys"][i] / 1000) /
                                                d["23"]["maxwh"] for i in range(0, len(d["23"]["Xs"]))])
threeLineP45, parm_cov = sp.optimize.curve_fit(model_func3Line, d["45"]["Ys"],
                                               [(d["45"]["maxwh"] - d["45"]["Xs"][i] * d["45"]["Ys"][i] / 1000) /
                                                d["45"]["maxwh"] for i in range(0, len(d["45"]["Xs"]))])
    ...
    if temp == 0 or temp == 23 or temp == 45:
        ...
        for j in range(0, len(Vs)):
            ...
            if i >= Ys[4]:
                ...
                if interpPerIncrement == "":
                    interpPerIncrement = model_func2_3Line(Ys[46], m1, b1, m2, b2, m3, b3, 2) / (len(Vs) - j)
                """Mode 3 is actually invalid. While in mode 3, the voltage drops so rapidly, buy the capacity in MaH doesent,
                so the equation giving the capcity in wh, which multiplies the two, actually starts INCREASING.

                To solve this, we will just linearly intropolate to 0 from the starting point of mode 3. """
                Xss.append(interpPerIncrement * (len(Vs) - j))
                # Xss.append(model_func2_3Line(i, m1,b1,m2,b2,m3,b3, 3)) #this does not work SOC actually increases
