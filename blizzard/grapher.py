...
def plotTripsOnDay(dbc, imei, sMonth, sDay, sYear):
    ...
    tripStartTimes, tripEndTimes, dists = get_trips(dbc, imei, curDate, end)
...
def plotMaxSpeedTripOnDay(dbc, imei, sMonth, sDay, sYear):
    ...
    stmt = "select * from imei{0} where stamp >= \"{1}\" and stamp <= \"{2}\" and latgps is not null and longgps is not null and longgps!=0 and latgps!=0  order by stamp".format(
        imei, curDate, end)
...
def plotDistanceVsDay(dbc, imei, sMonth, sDay, sYear, numDays):
    ...
    for i in range(0, numDays):
        ...
        for record in dbc.SQLSelectGenerator(query):
            ...
            if record[3] is not None and record[3] != 0:
                dists.append(record[3])
            else:
                dists.append(0)
        dday = 0
        cday = 0
