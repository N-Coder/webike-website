...
def detectTrips(dbc, imei, startDate, endDate):
    stmt = "select * from TT{0} where StartTime >= \"{1}\" and StartTime <= \"{2}\" order by StartTime".format(imei,
                                                                                                               startDate,
                                                                                                               endDate)
...
def detectChargingEvents(dbc, imei, startDate, endDate):
    stmt = "select stamp, ChargingCurr, batteryvoltage from imei{0} where stamp >= \"{1}\" and stamp <= \"{2}\" and chargingCurr is not null order by stamp;".format(
