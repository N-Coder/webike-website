...
def plotDistanceVsDayAGGREGATE(dbc, imeiList, sMonth, sDay, sYear, numDays):
    ...
    for j in sorted(d.keys()):
        ...
        ax.plot(Xs, d[j]["CDFYs"], label=jcount, color=colors[jcount],
                marker=markers[jcount % len(markers)])  # all stuff at end is to choose colors in order from colors dict
        jcount += 1
    ...
    ax.legend(numpoints=1, loc='best', fontsize=18, ncol=2, columnspacing=0, labelspacing=0, handletextpad=0,
              borderpad=.15, markerscale=0.99)
...
def plotChargeVsSOC(dbc, imeiList, sMonth, sDay, sYear, eMonth, eDay, eYear):
    ...
    for i in imeiList:
        ...
        for s in range(0, len(chargeStartVolts)):
            if chargeStartVolts[s] is not None and chargeEndVolts[s] is not None and chargeEndVolts[s] >= \
                    chargeStartVolts[s]:
...
def plotEmpiricalRange(dbc, imeiList, sMonth, sDay, sYear, eMonth, eDay, eYear):
    ...
    for i in imeiList:
        ...
        alltripSOCDeltas += map(operator.sub, [100 * a for a in SOC.SOCVals(23, tripStartVoltages)],
                                [100 * b for b in SOC.SOCVals(23, tripEndVoltages)])
...
def showSOCEstimates():
    ...
    ax3.legend(numpoints=1, ncol=3, loc='best', columnspacing=0, labelspacing=1, handletextpad=0, borderpad=.15,
               markerscale=2)
    # plt.ylim(0,1)
